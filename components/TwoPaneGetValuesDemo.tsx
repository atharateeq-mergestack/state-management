            ...
            <div className="space-y-6">
                <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        <input value={user?.name ?? ''} onChange={(e) => stateController.updateState({ user: { ...(user as any), name: e.target.value }, lastUpdated: new Date().toISOString() })} className="rounded border px-3 py-2" placeholder="Name" />
                        <input value={user?.email ?? ''} onChange={(e) => stateController.updateState({ user: { ...(user as any), email: e.target.value }, lastUpdated: new Date().toISOString() })} className="rounded border px-3 py-2" placeholder="Email" />
                        <input type="number" value={user?.age ?? 0} onChange={(e) => stateController.updateState({ user: { ...(user as any), age: Number(e.target.value) || 0 }, lastUpdated: new Date().toISOString() })} className="rounded border px-3 py-2" placeholder="Age" />
                    </div>
                </div>
                <div className="space-y-2">
...
export function TwoPaneGetValuesDemo() {
    return (
